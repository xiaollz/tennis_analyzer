...
4. 主动动作仅3个：蹬地转髋、腹部展开、胸肌按压(Press Slot)。其余全部被动。注意：胸肌按压不是"肘部往前推"（那是手臂发力），而是胸大肌将手臂"焊接"在躯干上，通过躯干旋转带动整个手臂框架前移。判断依据：图3-4中手臂是随躯干同步前移（胸肌连接正确）还是滞后于躯干然后猛追（胸肌脱节=手臂代偿的前兆）。
...
■ 症状群C：动力链脱节（手臂先于身体）
  表面症状：手臂和躯干同时启动或手臂先启动、肩膀抖动不稳、双臂不同步
  中层原因：力的传导在某个环节断裂
  深层根因（按可能性排序）：
    1. 击球太急/时机太早 → 没时间完成准备 → 手臂被迫提前抢跑
      诊断：球是在下落阶段被击中还是上升/最高点就被打了？
    2. 背部连接松掉 → 身体转但手臂没跟上 → 手臂自己补偿
      诊断：图2-3中肩膀是稳定平移还是在抖动调整？（抖动=连接差）
    3. 重心没在后脚 → 无法蹬地 → 髋无法领先 → 手臂抢跑
      诊断：图1中后脚是否承重？能不能抬起前脚？
    4. 左右臂不同步 → 左臂先收回+右臂后出发=两步动作不是一个旋转
      诊断：图2-5中非持拍手是和持拍手同步运动还是有明显时间差？
  建议侧重：
    - 根因1：口令`弹...落...打`三拍节奏，退后半步站位
    - 根因2：口令`背部一直托着不松`，drill=毛巾夹腋下打球
    - 根因3：口令`Unit Turn完成后能抬起左脚=重心对了`，drill=Right-Left-Hit节奏
    - 根因4：口令`左手松开的瞬间=右手出发的瞬间`，drill=双手握拍到松手即挥
    - 检验标准：肩膀是稳定的"钉子"（几乎不动），肘和手腕依次被甩出
