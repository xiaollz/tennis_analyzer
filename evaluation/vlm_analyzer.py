...
地基（L1-L3）问题会导致上层（L5-L7）做得再好也没用。先检查地基。

  L1 几何/空间    : 击球点 4 坐标（高度/横距/纵深/角度）+ 站位距离
  L2 时间/时序    : Unit Turn 时机、Bounce-Hit 节奏、加速时机
  L3 移动/步法    : Split Step、调整步、第一步方向
  L4 站姿         : 封闭/中性/半开放/完全开放、重心分布
  L5 身体姿态     : 脊柱倾角（后仰？）、头部稳定、肩线倾斜
  L6 上半身机制   : Unit Turn、X-Factor、胸推、Lat sling、左臂后压
  L7 手臂结构     : 击球臂形态、手腕 Lag、肘部位置、拍面角度
  L8 视觉/注意力  : 追球、四注视点、Quiet Eye、注意力分配
  L9 心智模型     : Receiving vs Sending、预设攻击区、自我对话
  L10 输出控制    : 球速/上旋/方向/深度的可控性

**关键原则：根因优先在 L1-L3，不要直接跳到 L6-L7 写一堆动作问题**。
如果击球点在身体侧方（L1 错），那胸不参与（L6 症状）只是结果，不是根因。

【L1 击球点几何 —— 极重要，单独说】

每次击球必须报告以下 4 个坐标的实际值（基于视频估算）：
  - 高度：膝盖以下 / 膝大腿之间 / 大腿到腰 / 腰到胸（理想） / 胸以上
  - 横距：< 30cm（贴身）/ 30-40cm（偏近）/ 40-60cm（理想） / 60-80cm / > 80cm（够不到）
  - 纵深：身体后方 / 身体侧方 / 前胯前方 30-50cm（理想）/ 前胯前方 60+ cm（太前）
  - 角度：< 30°（贴身打）/ 30-45° / 45° ⭐理想 / > 60°（太开）

如果四个坐标里有任何一个明显偏离理想 → **必须报告为 issue**，且根因优先级高于 L6。
...
严格JSON，不含其他文字：
{
  ...
  "contact_point": {
    "height": "膝下/膝大腿/大腿到腰/腰到胸（理想）/胸以上",
    "lateral_distance": "<30cm/30-40cm/40-60cm（理想）/60-80cm/>80cm",
    "depth": "身体后方/身体侧方/前胯前30-50cm（理想）/>60cm",
    "angle_degrees": "0-90 之间的实际估算值（理想 45°）",
    "verdict": "理想 / 部分对（哪些坐标对） / 完全偏离（往哪边偏）"
  },
  "spacing": "站位距离评价：人离球的落点合适吗？太近？太远？",
  "timing": {
    "unit_turn_timing": "球弹地时肩转角度估计（理想 90°）",
    "bounce_to_hit_ratio": "球弹起到击球时间是否合理",
    "acceleration_phase": "拍头加速峰值出现在哪一帧？理想是触球前 50ms"
  },
  "vision_attention": "追球/视线/注意力问题：击球瞬间是否能看清触球？头是否动？",
  "drills": [
    {
      "name": "训练名称",
